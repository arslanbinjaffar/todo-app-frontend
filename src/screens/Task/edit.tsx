...
const EditTask = () => {
    ...
    const {id} = useParams()
    const initialData = location.state ? location.state.find((state:any) => state._id == id):[];
    ...
    const handleSubmit = async(e:any) => {
        try {
            e.preventDefault();
            setLoading(true)
            const { data } = await axios.patch(`api/updatetask/${id}`, formData)
            setLoading(false)
            navigate("/tasks")
            toast(data.message)
        } catch (error) {
    ...
    return (
      ...
      <div className="grid gap-6 mb-6 md:grid-cols-1 bg-slate-400 p-4 md:w-[35%] w-[80%] rounded-lg">
            <h3 className="text-2xl font-bold text-center">Update Task</h3>
        <div>
          <label
            htmlFor="first_name"
